...
// for the second map, minor changes 
...
public abstract class Entity2 {
	...
	public Entity2(TileMap2 tm2) {
		tileMap2 = tm2;
		tileSize = tileMap2.getTileSize();
	...
	public boolean intersects(Entity2 o) {
	...
	public boolean validateNextPosition() {
		...
		if(left) {
			if(colTile == 0 || tileMap2.getType(rowTile, colTile - 1) == Tile.BLOCKED) {
				return false;
			}
			else {
				xdest = x - tileSize;
		...
		if(right) {
			if(colTile == tileMap2.getNumCols() || tileMap2.getType(rowTile, colTile + 1) == Tile.BLOCKED) {
				return false;
			}
			else {
				xdest = x + tileSize;
		...
		if(up) {
			if(rowTile == 0 || tileMap2.getType(rowTile - 1, colTile) == Tile.BLOCKED) {
				return false;
			}
			else {
				ydest = y - tileSize;
		...
		if(down) {
			if(rowTile == tileMap2.getNumRows() - 1 || tileMap2.getType(rowTile + 1, colTile) == Tile.BLOCKED) {
				return false;
			}
			else {
				ydest = y + tileSize;
